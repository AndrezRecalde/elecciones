...
public class calificador_ant extends HttpServlet {
                ...
                if (idacta > 0) {
                    ...
                    for (candidato c : listaCandidatos) {
                        ...
                        try {
                            ...
                            response.sendRedirect("error.jsp?men=Se encontraron datos no válidos!!<br>Ingrese nuevamente el acta.");
                            return;
                        }
                    }
                    for (candidato c : listaCandidatos) {
                        String votos = request.getParameter(Integer.toString(c.getIdcandidato())).toString();
                        acDB.CalificarActaCandidato(idacta, c.getIdcandidato(), Integer.parseInt(votos));
                    }
                    ...
                    response.sendRedirect("error.jsp?men=No se pudo ingresar el acta de " + dignidad + ": " + provincia + "/" + canton + "/" + parroquia + "/" + zona + "/" + junta + " ingresada con exito.");
                }
            } else {
