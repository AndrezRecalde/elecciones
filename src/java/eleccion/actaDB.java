...
public class actaDB {
    ...
    public acta CargarActaId(int idacta) {
        acta OBJ = new acta();
        Connection con = null;
        try {
            con = conexion.getConexion();
            String sql = "select a.idacta,a.fr_id_junta,a.fr_id_dignidad,"
                    + " a.cne_cod_acta,a.num_validos,a.num_no_voto,a.num_blancos,"
                    + " a.num_nulos,a.acta_usu_ing,a.acta_usu_mod,a.acta_fi,a.acta_um,"
                    + " a.cuadrada,a.legible,a.fr_id_acta_estado,ae.estado_nombre,d.nombre_dignidad, concat(j.num_junta,' ',genero) junta_nombre"
    ...
    public int TotalActasIngresadasRecinto(int iddignidad, int id_recinto) {
        int total = 0;
        Connection con = null;
        try {
            con = conexion.getConexion();
            String sql = "select count(*) as digitadas\n"
                    ...
                    + " where a.fr_id_dignidad=" + iddignidad + " and re.cod_recinto=" + id_recinto + " ;";
            Statement st = con.createStatement();
            ResultSet rs = st.executeQuery(sql);
            while (rs.next()) {
                total = rs.getInt("digitadas");
    ...
    public int TotalJuntasRecinto(int id_recinto) {
        int total = 0;
        Connection con = null;
        try {
            con = conexion.getConexion();
            String sql = "SELECT count(*) as total FROM junta as ju\n"
                    + "where ju.cod_recinto=" + id_recinto;
            Statement st = con.createStatement();
            ResultSet rs = st.executeQuery(sql);
            while (rs.next()) {
                total = rs.getInt("total");
