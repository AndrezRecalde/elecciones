...
import { onClearActas, onLoadActas, onLoading } from "../../store/eleccion/acta/actasResSlice";

export const useActasResStore = () => {
    const { actas, isLoading, pageLoad } = useSelector((state) => state.actasRes);
    const dispatch = useDispatch();



    const startLoadActas = async({ dignidad_id, canton_id, parroquia_id, tipo_acta }) => {
        try {
    ...
    const startLoadAllActas = async({ dignidad_id, canton_id, parroquia_id }) => {
        try {
            const { data } = await eleccionApi.post("/admin/listar/actas/todas", {
                dignidad_id,
                canton_id,
                parroquia_id
                });
